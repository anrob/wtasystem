...
class Contract < ActiveRecord::Base
  ...
  has_paper_trail :skip => [:confirmation, :prntkey23, :prntkey13, :tax_amount,:non_commissionable_charges,:base_price_of_act,:contract_status]
  require 'chronic'
  has_one :actcode
...
def self.mailchimp
 ...
 def caltime
   self.date_of_event.strftime("%Y%m%d")
 end
 def caltimes
